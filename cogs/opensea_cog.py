...
class OpenseaCog(commands.Cog):
    ...
    async def os(self, interaction: discord.Interaction, collection: str):
        ...
        if type(collection_details) == dict:
            ...
            embed.add_field(name="Floor Price", value=stats.get("floor price"))
            ...
            embed.add_field(
                name="Website", value=f"[Visit]({website})", inline=False)
            embed.add_field(name="Twitter", value=f"[View]({twitter})")
            embed.add_field(name="Discord", value=f"[Join]({discord_server})")
